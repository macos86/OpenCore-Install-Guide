...
| :--- | :--- |
| **`0300220D`** | Utilizzato quando l'iGPU desktop viene utilizzato per guidare un display |
| **`04001204`** | Utilizzato quando Desktop iGPU viene utilizzato solo per attività di elaborazione e non gestisce un display |
| **`07002216`** | Utilizzato quando usiamo una iGPU Desktop Broadwell |

Ho aggiunto anche un'altra parte che mostra un falso "id-dispositivo" nel caso in cui tu abbia un HD 4400 che non è supportato in macOS.

Il falso ID dispositivo è impostato in questo modo:

* `12040000` - questo è l'ID del dispositivo per HD 4600 che ha il supporto in macOS

Aggiungiamo anche altre 3 proprietà, `framebuffer-patch-enable`, `framebuffer-stolenmem` and `framebuffer-fbmem`. The first enables patching via WhateverGreen.kext, the second sets the min stolen memory to 19MB e il terzo imposta la memoria del framebuffer su 9 MB. di solito non è necessario, poiché può essere configurato nel BIOS (64 MB consigliati) ma richiesto quando non è disponibile.

* **Nota**: i framebuffer headless (dove la dGPU è il display in uscita) non sono necessari `framebuffer-patch-enable`, `framebuffer-stolenmem` and `framebuffer-fbmem`
...
(Questo è un esempio per un desktop HD 4400 senza dGPU e senza opzioni BIOS per la memoria iGPU)
...
(Questo è un esempio per un desktop Iris Pro 6200 senza opzioni BIOS per la memoria iGPU)
...
| **-wegnoegpu** | Disabilita tutte le altre GPU tranne la integrata Intel, utile per disabiltare le dGPU |
...
Per questo esempio Haswell, sceglieremo iMac15,1 SMBIOS. La ripartizione tipica è la seguente:

| SMBIOS | Hardware |
| :--- | :--- |
| iMac14,4 | Haswell con solo iGPU |
| iMac15,1 | Haswell con dGPU |
| iMac16,2 | Broadwell |
...
::: tip Nota
Per avviare macOS Monterey hai bisogno di questi SMBIOS:

> Sceglierli per Big Sur o sistemi meno recenti non è raccomandato, dato che la Gestione Energetica non sarebbe ottimizzata.

| SMBIOS | Hardware |
| :--- | :--- |
| iMac16,2 | Broadwell solo con iGPU |
| iMac17,1 | Broadwell con dGPU |
...
* **IgnoreInvalidFlexRatio**: YES
  * Risolvw il problema quando MSR_FLEX_RATIO (0x194) non può essere disabilitato nel BIOS, richiesto per tutti i sistemi basati su Skylake precedenti

...
uesta sezione consente il passaggio di spazi a macOS che vengono generalmente ignorati, utile se abbinato a `DevirtualiseMmio`

### Quirks

::: tip Info
Impostazioni relative alle patch boot.efi e alle correzioni del firmware, a seconda di dove la tua scheda ha UEFI, hai 2 opzioni a seconda di cosa supporta la tua scheda madre:
...
| :--- | :--- | :--- |
| AvoidRuntimeDefrag | No | Big Sur potrebbe richiedere che questa stranezza sia abilitata |
| EnableSafeModeSlide | No | |
| EnableWriteUnprotector | No | |
| ProvideCustomSlide | No | |
| RebuildAppleMemoryMap | Yes | Ciò è necessario per avviare OS X da 10.4 a 10.6 |
| SetupVirtualMap | No | |
...
* **AvoidRuntimeDefrag**: NO
  * Risolve i servizi di runtime UEFI come data, ora, NVRAM, controllo dell'alimentazione;
  * macOS Big Sur, tuttavia, richiede la presenza della tabella APIC, altrimenti causerebbe i primi panic del kernel, quindi questa stranezza è consigliata per quegli utenti.
...
* **XhciPortLimit**: YES
  * Questa è attualmente la patch del limite di 15 porte, non fare affidamento su di essa perché non è una soluzione garantita per riparare USB. Crea un file [USB map](/OpenCore-Post-Install/usb/) quando possibile.
...
* **SecureBootModel**: Default
  * Abilita la funzionalità di avvio sicuro di Apple in macOS, fare riferimento alla sezione [Security](/OpenCore-Post-Install/universal/security.md) per ulteriori informazioni.
...
* **rtc-blacklist**: <>
  * Da utilizzare insieme a RTCMemoryFixup, vedere qui per maggiori informazioni: [Risolvere i problemi di scrittura RTC](/OpenCore-Post-Install/misc/rtc.md#finding-our-bad-rtc-region)
...
Per questo esempio Clarkdale , sceglieremo iMac11,2 SMBIOS - questo viene fatto intenzionalmente per motivi di compatibilità. Esistono tre SMBIOS principali utilizzati per  Clarkdale:

| SMBIOS | Hardware |
| :--- | :--- |
| iMac11,1 | Lynnfield SMBIOS |
| iMac11,2 | Clarkdale SMBIOS |
| MacPro6,1 |  Mojave e SMBIOS più recenti|
...
Relativo al passthrough della tastiera boot.efi utilizzato per FileVault e supporto Hotkey, lascia tutto qui come predefinito oltre a:

| Quirk | Value | Comment |
| :--- | :--- | :--- |
| KeySupport | NO | Abilita se il tuo BIOS supporta UEFI |

...
Per impostazione predefinita, Sample.plist ha questa sezione impostata per iGPU e audio. I driver iGPU di Iron Lake di Apple non supportano l'hardware desktop, quindi PciRoot `PciRoot(0x0)/Pci(0x2,0x0)` può essere rimosso dalla sezione `Aggiungi`. Per l'audio imposteremo il layout nella sezione boot-args, quindi la rimozione di `PciRoot(0x0)/Pci 0x1b, 0x0)` è consigliata anche dalle sezioni `Add` e` Block`

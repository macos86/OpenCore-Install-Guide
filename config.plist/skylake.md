...
| :--- | :--- |
| **`00001219`** |  Utilizzato quando l'iGPU desktop viene utilizzato per guidare un display |
| **`01001219`** |  Utilizzato quando Desktop iGPU viene utilizzato solo per attività di elaborazione e non gestisce un display |
...
E nota aggiuntiva per gli utenti P530, la tua iGPU non è supportata in modo nativo, quindi dovrai aggiungere la seguente proprietà:

| Key | Type | Value |
| :--- | :--- | :--- |
| device-id | Data | `1B190000` |

Una volta che hai finito, dovresti avere qualcosa di simile:
...
(Questo è un esempio per un desktop HD P530 senza dGPU e senza opzioni BIOS per la memoria iGPU)
:::
...
* Puoi eliminare completamente questa proprietà poiché al momento non la utilizzeremo
...
| **nvda_drv_vrl=1** | Utilizzato per abilitare i Web Driver di NVIDIA su schede Maxwell e Pascal su Sierra e High Sierra |
...
Per questo esempio Skylake , sceglieremo iMac17,1 SMBIOS.
